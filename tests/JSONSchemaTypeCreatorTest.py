...
def test_create_and_lookup_name():
    ...
    assert 'Test' == DataSourceTypeCreator.lookup_type(annotated_schema).__name__


def test_create_and_lookup_type():
    cut = DataSourceTypeCreator('Test', {'type': 'object'}, module=__name__)
    annotated_schema = cut.annotate()
    assert issubclass(DataSourceTypeCreator.lookup_type(annotated_schema), DataSource)
...
def test_create_property_is_informational():
    ...
    annotated_schema = cut.annotate()
    typ = DataSourceTypeCreator.lookup_type(annotated_schema)
...
def test_create_property_type_is_ObjectProperty_with_DatatypeProperty():
    ...
    annotated_schema = cut.annotate()
    typ = DataSourceTypeCreator.lookup_type(annotated_schema, '/properties/toast')
