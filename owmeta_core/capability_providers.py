...
class TransactionalDataSourceDirProvider(OutputFilePathProvider):
    ...
    def provides_to(self, obj):
        if isinstance(obj, DataSource):
            ...
            return type(self).Helper(self._basedir, key, self.transaction_manager)
        return None
        ...
        def tpc_abort(self, transaction):
            try:
                ...
                L.error('Received exception in tpc_abort for transaction for %s,'
                        ...
                        self._committed_path, self._uncommitted_path,
                        self._prev_version_path,
                        exc_info=True)
            finally:
